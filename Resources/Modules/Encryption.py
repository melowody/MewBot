...
class Encryption:
    ...
    async def a85d(self, ctx, *args):
        if(args != ()):
            emb = (discord.Embed(color=0xf7b8cf))
            emb.add_field(name="Ascii85 Decoded Message", value=base64.a85decode(''.join(args).encode(), adobe=True).decode() if args.endswith('~>') else base64.a85decode(args.encode()).decode())
            await ctx.send(embed=emb)
        else:
            await ctx.send("You need to type something to decode!")
